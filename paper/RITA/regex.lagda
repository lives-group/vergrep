...
\Archive{RESEARCH ARTICLE} % Type of Article: Researh, Review, Tutorial, Study Case

\PaperTitle{Certified Derivative-Based Parsing of Regular Expressions} % Article title - English

\ShortTitle{Certified Derivative-Based Parsing of Regular Expressions} % Short Article title - English

\Titulo{Verificação Formal de Parsing Baseado em Derivadas de Expressões Regulares} % Article title - Portuguese
...
\Authors{Rodrigo Ribeiro 1\textsuperscript{1}*, Raul Lopes 2\textsuperscript{2},  Carlos Camar\~ao 3\textsuperscript{3}} % Authors
\affiliation{\textsuperscript{1}\textit{Departamento de Computação, Universidade Federal de Ouro Preto, Brazil}} % Author affiliation
\affiliation{\textsuperscript{2}\textit{Departamento de Computação, Universidade Federal de Ouro Preto, Brazil}} % Author affiliation
\affiliation{\textsuperscript{3}\textit{Departamento de Ciência da Computação, Universidade Federal de Minas Gerais, Brazil}} % Author affiliation
\affiliation{*\textbf{Corresponding author}: rodrigo.ribeiro\verb|@@|ufop.edu.br} % Corresponding author

\Keywords{Regular Expressions --- Derivatives --- Type Theory --- Dependent Types --- Agda}

\newcommand{\keywordname}{Keywords} % Defines the keywords heading name

\PalavrasChave{Expressões Regulares --- Derivadas --- Teoria de Tipos --- Tipos Dependentes --- Agda} 
...
\Resumo{
   Nesse trabalho apresentamos a formalização de algoritmos para parsing
   de expressões regulares utilizando as derivadas de Brzozowski e de Antimirov,
   na linguagem Agda. O algoritmo produz, como resultado, uma prova de que a
   string de entrada é aceita por uma dada expressão regular ou que essa não
   pertence a linguage dessa expressão. Usamos o algoritmo formalizado para
   a implementação de uma ferramenta de busca de texto, similar ao conhecido
   GNU grep. Finalmente, relatamos experimentos conduzidos com a ferramenta
   desenvolvida.
}
...
%format inj1 = "\C{inj_1}"
%format inj2 = "\C{inj_2}"
%format _<U>_    = "\D{\_\lor\_}"
%format <U> = "\D{\lor}"

\begin{code}
   data _<U>_ (A B : Set) : Set where
     inj1 : A -> A <U> B
     inj2 : B -> A <U> B
\end{code}    
Note that each constructor of the previous data type represents how
we can build evidence for the proposition |A <U> B|: using constructor
|inj1| together with an evidence for |A| or using constructor |inj2| and 
evidence for |B|. Intuitively, type |_<U>_| encodes the intuitionistic intepretation
...
used in our formalization use this type. The type |not P| is an
...
data Parity : Nat -> Set where
   ...
   Odd  : forall {n : Nat} -> Parity (succ (n + n))
...
%format ]] = "\D{\rrbracket}"
%format ++ = "\F{++}"
%format _>_=>_ = "\C{\_\bullet\_\Rightarrow\_}"
%format => = "\C{\Rightarrow}"
%format _+L_ = "\C{\_+L\_}"
%format _+R_ = "\C{\_+R\_}"
%format +L = "\C{+L}"
...
%format choice-invert = "\F{+invert}"
\begin{spec}
 choice-invert : forall {xs l r} -> xs <<-[[ l + r ]] -> xs <<-[[ l ]] <U> xs <<-[[ r ]]
 choice-invert (r +L pr) = inj1 pr
 choice-invert (l +R pr) = inj2 pr    
\end{spec}    
...
Intuitively, function |choice-invert| specifies that, in the case that |xs <<-[[ l + r ]]|,
...
or not the empty string (by returning |Eps| or |Emp|, respectively):
\[
    \begin{array}{lcl}
         \nu(\emptyset) & = & \emptyset \\
         \nu(\epsilon)    & = & \epsilon \\
...
%format proj2 = "\F{\pi_2}"
%format choice-invert = "\F{+-invert}"
%format cat-invert = "\F{\bullet -invert}"
%format :*: = "\F{\circ}"
...
\begin{spec}
  nuu[_] : forall (e : Regex) -> Dec ( [] <<-[[ e ]])
  nuu[ Emp ]> = no (\ ())
  nuu[ Eps ]> = yes Eps
  nuu[ # x ]> = no (\ ())
  nuu[ e :.: e' ]> with nuu[ e ]> | nuu[ e' ]>
  nuu[ e :.: e' ]> | yes pr | (yes pr1) = yes (pr > pr1 => refl)
  nuu[ e :.: e' ]> | yes pr | (no pr1) = no (pr1 :*: proj2 :*: cat-invert)
  nuu[ e :.: e' ]> | no pr | pr1 = no (pr :*: proj1 :*: cat-invert)
  nuu[ e + e' ]> with nuu[ e ]> | nuu[ e' ]>
...
RE semantics. Lemma |cat-invert| states that if the empty string
is in the language of |l :.: r| (where |l| and |r| are arbitrary REs)
then the empty string belongs to |l| and |r|'s languages.
Lemma |choice-invert| is defined similarly for union.
...
of them is empty (|Emp|) the result is also empty, and the empty string is the 
...
\begin{spec}
  ...
  e <:*: e' = e :*: e'
\end{spec}
For Kleene star both |Emp| and |Eps| are replaced by |Eps|.
\begin{spec}
  _:⋆ : Regex -> Regex
  Emp :⋆ = Eps
  Eps :⋆ = Eps
...
\begin{proof}
  ...
  |Emp|. If |e = Emp|, then |Emp :+ e' = e'| and
  the desired result follows. The same reasoning applies for |e'= Emp|.
      \item[($\leftarrow$)]: By case analysis on the structure of |e|, |e'|. The
   only interesting cases are when one of the REs is |Emp|.  If
   |e = Emp|, we need to analyse the structure of
   |xs <<-[[ e + e' ]]|. The result follows directly or by
   contradiction using |xs <<-[[ Emp ]]|. The same reasoning
   applies when |e' = Emp|.
  \end{itemize}          
\end{proof}
...
\begin{proof}
  ...
  |Eps| or |Emp|. When some of the REs are equal to
  |Emp|, the result follows by contradiction. If one of the REs
...
\begin{spec}
  ...
  ...| no prf = Emp
  d[ e :*: e' , c ]> with nuu[ e ]>
  d[ e :*: e' , c ]> | yes pr = (d[ e , c ]> <:*: e') :+ d[ e' , c ]> 
  d[ e :*: e' , c ]> | no pr = d[ e , c ]> <:*: e'
  d[ e + e' , c ]> = d[ e , c ]> :+ d[ e' , c ]>
  d[ e ⋆ , c ]> = d[ e , c ]> <:*: (e `⋆)
\end{spec}
From this definition we prove the following important properties of
the derivative operation. Soundness of |d[_,_]| ensures that if a
string |xs| is in the language of |d[ e , x ]> |, then
...
| xs <<-[[ d[ e , x ]> ]] | holds then |(x :: xs) <<-[[ e ]] | holds.
\end{Theorem}
\begin{proof}
...
For all REs |e|, all strings |xs| and all symbols |x|, if
|(x :: xs) <<-[[ e ]] | holds then | xs <<-[[ d[ e , x ]> ]] | holds.
\end{Theorem}
\begin{proof}
...
\begin{Lemma}\label{wopeq}
  For all sets of REs |S|, all REs |e| and all strings |s|, if |s <<-<[ e ** S ]]> | holds then there exist |s1| and |s2| such that
  |s == s1 ++ s2|, |s1 <<-<[ S ]]> | and |s2 <<-[[ e ]]| holds.
\end{Lemma}
\begin{proof}
...
\begin{Lemma}[Lemma |nIsPrefix|]\label{pref1}
...
\begin{Lemma}[Lemma |nIsPrefix-::|]\label{pref2}
  For all REs |e| and all strings |xs|, if |[] <<-[[ e ]]| and |IsPrefix xs d[ e , x ]> | do not hold then
...
\begin{Lemma}[Lemma |nIsSubstring|]\label{sub1}
...
\begin{Lemma}[Lemma |nIsSubstring-::|]
...
%format d-sound = "\F{\partial-sound}"
%format cong = "\F{cong}"
...
prefix. Otherwise, it verifies, for each symbol |x|, whether RE |d[ e , x ]>|
...
\begin{spec}
  IsPrefixDec : forall (xs : List Char)(e : Regex) -> Dec (IsPrefix xs e)
  IsPrefixDec [] e with nuu[ e ]>
  IsPrefixDec [] e | yes p = yes (Prefix [] [] refl p)
  IsPrefixDec [] e | no p = no (nIsPrefix p)
  IsPrefixDec (x :: xs) e with nuu[ e ]>
  IsPrefixDec (x :: xs) e | yes p = yes (Prefix [] (x :: xs) refl p)
  IsPrefixDec (x :: xs) e | no p with IsPrefixDec xs (d[ e , x ])
  IsPrefixDec (x :: xs) e | no p | (yes (Prefix ys zs eq wit))
    = yes (Prefix (x :: ys) zs (cong (_::_ x) eq) (d-sound _ _ _ wit))
  IsPrefixDec (x :: xs) e | no pn | (no p) = no (nIsPrefix-:: pn p)
\end{spec}
...
\begin{spec}
  IsSubstringDec : forall (xs : List Char)(e : Regex) -> Dec (IsSubstring xs e)
  IsSubstringDec [] e with nuu[ e ]>
  IsSubstringDec [] e | yes p = yes (Substring [] [] [] refl p)
  IsSubstringDec [] e | no p = no (nIsSubstring (nIsPrefix p))
  ...
  IsSubstringDec (x :: xs) e | no p1 | (no p)
    = no (nIsSubstring-:: p1 p)
\end{spec}
